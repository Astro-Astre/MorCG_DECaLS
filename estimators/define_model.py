...
from estimators import efficientnet_standard, efficientnet_custom, custom_layers
...
class GenericLightningModule(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        predictions = self(x)
        loss = torch.mean(self.loss_func(predictions, labels))
        self.log("val/supervised_loss", loss, on_step=self.log_on_step, on_epoch=True, prog_bar=True, logger=True,
                 sync_dist=True)
        if predictions.shape[1] == 2:  # will only do for binary classifications
            # logging.info(predictions.shape, labels.shape)
    ...
    def test_step(self, batch, batch_idx):
        ...
        predictions = self(x)
        loss = torch.mean(self.loss_func(predictions, labels))
        self.log("test/supervised_loss", loss, on_step=self.log_on_step, on_epoch=True, prog_bar=True, logger=True,
                 sync_dist=True)
        return loss
